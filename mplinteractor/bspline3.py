...
def bspline3(knot, control, t, clamp=False, nderiv=0):
  ...
  f = nx.zeros(t.shape)
  df = nx.zeros(t.shape)
  d2f = nx.zeros(t.shape)
  d3f = nx.zeros(t.shape)
...
def demo():
  import pylab
  t = nx.linspace(-1,7,40 );
  knot = nx.array([0, 1, 1, 3, 4, 6],'f')
  #knot = nx.array([0, 0, 1, 4, 5, 5],'f')
  control = nx.array([1, 2, 3, 2, 1, 2],'f') 
